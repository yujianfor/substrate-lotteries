use sp_state_machine::remote_ext::Builder;
...
pub(crate) type Staking = crate::Module<Runtime>;
...
impl pallet_session::Trait for Runtime {
	...
	type ValidatorIdOf = crate::StashOf<Self>;
...
impl pallet_session::historical::Trait for Runtime {
	type FullIdentification = crate::Exposure<<Self as frame_system::Trait>::AccountId, Balance>;
	type FullIdentificationOf = crate::ExposureOf<Self>;
}
...
fn test_runtime_works() {
	...
	Builder::new()
		...
		.build()
		.execute_with(|| {
				assert_eq!(Staking::validator_count(), 400);
		});
}
...
use crate::{Validators, Nominators, Nominations, SlashingSpans, Store};
		...
		.execute_with(|| {
			...
			let nominator_votes = <Nominators<Runtime>>::iter().map(|(nominator, nominations)| {
				...
				targets.retain(|stash| {
					<Staking as Store>::SlashingSpans::get(&stash).map_or(
						true,
			...
			let phragmms_score = {
				let PrimitiveElectionResult { winners, assignments } = sp_npos_elections::balanced_heuristic::<AccountId, Perbill>(
